  ...
  describe("when caller = EOA", () => {
    ...
    before(async () => {
      ...
      await recipientUP
        ...
        .setData(
          lsp17ExtensionDataKeyForGraffiti,
          await graffitiExtension.getAddress()
        );
    ...
    describe("when recipient = EOA", () => {
      describe("when transferring value via `execute(...)`", () => {
        describe("when transferring value without bytes `_data`", () => {
          ...
          it("should fail when controller has permission CALL only but not TRANSFERVALUE", async () => {
            ...
            const initialBalanceRecipient = await provider.getBalance(
              recipient.address
            );
            ...
            await expect(
              context.keyManager.connect(canCallOnly).execute(transferPayload)
            )
            ...
            const newBalanceRecipient = await provider.getBalance(
              recipient.address
            );
          ...
          it("should fail when controller has neither CALL nor TRANSFERVALUE permissions", async () => {
            ...
            await expect(
              ...
              .withArgs(
                canNeitherCallNorTransferValue.address,
                "TRANSFERVALUE"
              );
      ...
      describe("when transferring value via `executeRelayCall(...)`", () => {
        it("should revert if tx was signed with Eth Signed Message", async () => {
          ...
          const signature = await context.mainController.signMessage(
            encodedMessage
          );
    ...
    describe("when recipient is a UP", () => {
      describe("when transferring value with graffiti `_data` (prefixed with `bytes4(0)`)", () => {
        ...
        it("should pass when controller has permission TRANSFERVALUE + CALL", async () => {
          ...
          const transferPayload = universalProfileInterface.encodeFunctionData(
            "execute",
            [OPERATION_TYPES.CALL, recipientUP.target, amount, data]
          );
  ...
  describe("when caller has SUPER_TRANSFERVALUE + CALL", () => {
    ...
    describe("when sending native tokens with `data`", () => {
      ...
      it("should allow to send LYX with some `data` to an EOA listed in the AllowedCalls", async () => {
        ...
        const transferLyxPayload = universalProfileInterface.encodeFunctionData(
          "execute",
      ...
      it("should allow to send LYX with some `data` to a contract listed in the AllowedCalls", async () => {
        ...
        const transferLyxPayload = universalProfileInterface.encodeFunctionData(
          "execute",
          [OPERATION_TYPES.CALL, lyxRecipientContract.target, amount, data]
        );
    ...
    describe("when interacting with some contracts", () => {
      it("should not be allowed to interact with a disallowed LSP7 contract", async () => {
        const newLSP7Token = await new LSP7Mintable__factory(
          context.accounts[0]
        ).deploy(
          "New LSP7 Token",
          "LSP7TKN",
        ...
        const lsp7TransferPayload = newLSP7Token.interface.encodeFunctionData(
          "transfer",
          [
            await context.universalProfile.getAddress(),
        ...
        const executePayload = universalProfileInterface.encodeFunctionData(
          "execute",
          [OPERATION_TYPES.CALL, newLSP7Token.target, 5, lsp7TransferPayload]
        );
      ...
      it("should be allowed to interact with an allowed LSP7 contract", async () => {
        ...
        const executePayload = universalProfileInterface.encodeFunctionData(
          "execute",
          [OPERATION_TYPES.CALL, lsp7Token.target, 0, lsp7TransferPayload]
        );
        ...
        expect(lsp7SenderBalanceAfter).to.equal(
          lsp7SenderBalanceBefore - tokenAmount
        );

        expect(lsp7RecipientBalanceAfter).to.equal(
          lsp7RecipientBalanceBefore + tokenAmount
        );
      });
      ...
      it("should be allowed to interact with an allowed contract", async () => {
        ...
        const payload = universalProfileInterface.encodeFunctionData(
          "execute",
          [
            OPERATION_TYPES.CALL,
            await targetContract.getAddress(),
      ...
      it("should be allowed to interact with an allowed contract + send some LYX while calling the function", async () => {
        ...
        const targetContractPayload =
          targetContract.interface.encodeFunctionData("updateState", [
            newValue,
          ]);
      ...
      describe("eg: any TargetContract", () => {
        for (let ii = 1; ii <= 5; ii++) {
          it(`TargetContract nb ${ii}`, async () => {
            ...
            const payload = targetContract.interface.encodeFunctionData(
              "setNumber",
              [newValue]
            );
      ...
      describe("eg: any LSP7 Token owned by the UP", () => {
        for (let ii = 1; ii <= 5; ii++) {
          it(`LSP7DigitalAsset nb ${ii}`, async () => {
            const lsp7Token = await new LSP7Mintable__factory(
              context.accounts[0]
            ).deploy(
              "LSP7 Token",
              "LSP7",
            ...
            const tokenTransferPayload = lsp7Token.interface.encodeFunctionData(
              "transfer",
              [
                await context.universalProfile.getAddress(),
                tokenRecipient,
            ...
            const executePayload = universalProfileInterface.encodeFunctionData(
              "execute",
              [OPERATION_TYPES.CALL, lsp7Token.target, 0, tokenTransferPayload]
            );
            ...
            expect(senderTokenBalanceAfter).to.equal(
              senderTokenBalanceBefore - tokenAmount
            );
            expect(recipientTokenBalanceAfter).to.equal(
              recipientTokenBalanceBefore + tokenAmount
            );
          });
        }
      });
