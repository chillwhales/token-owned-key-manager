  ...
  describe("when caller is a contract", () => {
    ...
    describe("> contract calls", () => {
      it("should allow to set a key hardcoded inside a function of the calling contract", async () => {
        ...
        const initialStorage = await context.universalProfile.getData(
          hardcodedDataKey
        );
        ...
        const newStorage = await context.universalProfile.getData(
          hardcodedDataKey
        );
    ...
    describe("> Low-level calls", () => {
      it("Should allow to `setHardcodedKeyRawCall` on UP", async () => {
        ...
        const result =
          await contractCanSetData.setHardcodedKeyRawCall.staticCall({
    ...
    describe("when trying to set a disallowed key", () => {
      for (let ii = 1; ii <= 5; ii++) {
        const key = ethers.keccak256(
          ethers.toUtf8Bytes(`dissallowed key ${ii}`)
        );
