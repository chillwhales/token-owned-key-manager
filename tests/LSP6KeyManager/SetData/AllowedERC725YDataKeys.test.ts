...
export const shouldBehaveLikeAllowedERC725YDataKeys = (
          ...
          describe("should pass when", () => {
            it("input contains all the allowed keys as DUPLICATE", async () => {
              ...
              const values = [
                ...
                ethers.hexlify(ethers.toUtf8Bytes("Some Data for customKey3")),
                ethers.hexlify(
                  ethers.toUtf8Bytes("Some Data (override 1) for customKey2")
                ),
                ethers.hexlify(
                  ethers.toUtf8Bytes("Some Data (override 1) for customKey3")
                ),
                ethers.hexlify(
                  ethers.toUtf8Bytes("Some Data (override 2) for customKey2")
                ),
                ethers.hexlify(
                  ethers.toUtf8Bytes("Some Data (override 1) for customKey4")
                ),
                ethers.hexlify(
                  ethers.toUtf8Bytes("Some Data (override 2) for customKey3")
                ),
                ethers.hexlify(
                  ethers.toUtf8Bytes("Some Data (override 2) for customKey4")
                ),
              ];
      ...
      describe("when setting one key", () => {
        it("should pass when setting any random Mapping key", async () => {
          const randomMappingKey =
            "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa00000000000000000000000011111111";
          const randomMappingValue = ethers.hexlify(
            ethers.toUtf8Bytes("Random Mapping Value")
          );
  ...
  describe("Testing bytes32(0) (= zero key) edge cases", () => {
    ...
    describe("When bytes32(0) key is part of the AllowedERC725YDataKeys", () => {
        ...
        (testCase) => {
          it(`should pass when setting a data key listed in the allowed ERC725Y data keys: ${testCase.allowedDataKey}`, async () => {
            ...
            const value = ethers.hexlify(
              ethers.toUtf8Bytes("some value for " + key)
            );
      ...
      [
        {
          datakeyToSet: ethers.keccak256(
            ethers.toUtf8Bytes("Some random data key 1")
          ),
        },
        {
          datakeyToSet: ethers.keccak256(
            ethers.toUtf8Bytes("Some random data key 2")
          ),
        },
        {
          datakeyToSet: ethers.keccak256(
            ethers.toUtf8Bytes("Some random data key 3")
          ),
        },
        {
          datakeyToSet: ethers.keccak256(
            ethers.toUtf8Bytes("Some random data key 4")
          ),
        },
        {
          datakeyToSet: ethers.keccak256(
            ethers.toUtf8Bytes("Some random data key 5")
          ),
        },
      ].forEach((testCase) => {
