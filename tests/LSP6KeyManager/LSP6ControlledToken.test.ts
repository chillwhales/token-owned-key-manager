...
const buildContext = async () => {
  ...
  await lsp7
    .connect(accounts[0])
    .transferOwnership(await keyManager.getAddress());
...
describe("When deploying LSP7 with LSP6 as owner", () => {
  ...
  describe("when trying to call mint(..) function on  in LSP7 through LSP6", () => {
    it("should revert because function does not exist on LSP6", async () => {
      ...
      await expect(
        context.keyManager.connect(context.mainController).execute(mintPayload)
      )
  ...
  describe("using transferOwnership(..) in LSP7 through LSP6", () => {
    before(
      "previous token contract's ownership was renounced, build new context",
      async () => {
        context = await buildContext();
      }
    );
    ...
    it("`setData(...)` -> should revert with 'caller is not the owner' error.", async () => {
      ...
      await expect(
        ...
        .withArgs(context.keyManager.getAddress());
    });
    ...
    it("should allow the new owner to call mint(..)", async () => {
      ...
      await LSP7.connect(newOwner).mint(
        context.mainController.address,
        1,
        true,
        "0x"
  ...
  describe("using setData(..) in lSP7 through LSP6", () => {
    ...
    describe("testing SETDATA permission", () => {
      const firstRandomSringKey = ethers.keccak256(
        ethers.toUtf8Bytes("FirstRandomString")
      );
      const secondRandomSringKey = ethers.keccak256(
        ethers.toUtf8Bytes("SecondRandomString")
      );
      const notAllowedKey = ethers.keccak256(
        ethers.toUtf8Bytes("Not Allowed ERC725Y data key")
      );
    ...
    describe("when trying to call execute(..) function on LSP7 through LSP6", () => {
      it("should revert because function does not exist on LSP7", async () => {
        ...
        const mintPayload = newTokenContract.interface.encodeFunctionData(
          "mint",
          [context.mainController.address, 1000, true, "0x"]
        );

        const payload =
          LSP0ERC725Account__factory.createInterface().encodeFunctionData(
            "execute",
            [0, await newTokenContract.getAddress(), 0, mintPayload]
          );
