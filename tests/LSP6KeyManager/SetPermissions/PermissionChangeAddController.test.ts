  ...
  before("setup", async () => {
    ...
    const firstSetupPermissionsKeys = [
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasAllPermissionsTestCase.addressWith16BytesHexPermissionsLength.substring(
          2
        ),
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasAllPermissionsTestCase.addressWith40BytesHexPermsissionsLength.substring(
          2
        ),
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasAddControllerTestCase.addressWith16BytesHexPermissionsLength.substring(
          2
        ),
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasAddControllerTestCase.addressWith40BytesHexPermsissionsLength.substring(
          2
        ),
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasEditPermissionsTestCase.addressWith16BytesHexPermissionsLength.substring(
          2
        ),
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasEditPermissionsTestCase.addressWith40BytesHexPermsissionsLength.substring(
          2
        ),
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasSetDataTestCase.addressWith16BytesHexPermissionsLength.substring(
          2
        ),
      ERC725YDataKeys.LSP6["AddressPermissions:Permissions"] +
        callerHasSetDataTestCase.addressWith40BytesHexPermsissionsLength.substring(
          2
        ),
    ];
      ...
      describe("when caller is an address with ALL PERMISSIONS", () => {
        ...
        describe("when editing `AddressPermissions[]` array length", () => {
          it("should be allowed to increment the length", async () => {
            ...
            const newLength = ethers.toNumber(
              ethers.toBigInt(currentLength) + BigInt(1)
            );
    ...
    describe("when setting multiple keys", () => {
      ...
      describe("when caller is an address with permission SETDATA + ADDCONTROLLER + 3x Allowed ERC725Y data keys", () => {
        ...
        it("(should pass): 2 x allowed data keys + add 2 x new controllers + increment AddressPermissions[].length by +2", async () => {
          const currentPermissionsArrayLength =
      ...
      describe("when caller is an address with permission SETDATA + EDITPERMISSIONS + 3x Allowed ERC725Y data keys", () => {
        ...
        it("(should fail): 2 x allowed data keys + increment AddressPermissions[].length by +1", async () => {
          ...
          const newArrayLength = ethers.toNumber(
            ethers.toBigInt(currentArrayLength) + BigInt(1)
          );
        ...
        it("(should fail): edit permissions of 2 x existing controllers + (set 1 x allowed data key) + (set 1 x NOT allowed data key)", async () => {
          ...
          const values = [
            ...
            ethers.hexlify(ethers.toUtf8Bytes("My First Value")),
            ethers.hexlify(
              ethers.toUtf8Bytes("Random data for not allowed value")
            ),
          ];
