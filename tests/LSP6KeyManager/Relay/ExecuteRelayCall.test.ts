  ...
  describe("`executeRelayCall(..)`", () => {
    ...
    before(async () => {
      ...
      const allPermissionsWithoutExecuteRelayCall = ethers.zeroPadValue(
        ethers.toBeHex(
          ethers.toBigInt(ALL_PERMISSIONS) -
            ethers.toBigInt(PERMISSIONS.EXECUTE_RELAY_CALL)
        ),
        32
      );
    ...
    describe("When testing signed message", () => {
          ...
          it("should pass if signer has the `to` address in its allowed calls", async () => {
            ...
            expect(tx)
              ...
              .withArgs(
                ...
                signedMessageParams.msgValue,
                context.universalProfile.interface.getFunction("execute")
                  .selector
              );
            ...
            expect(balanceOfUpAfter).to.equal(
              balanceOfUpBefore + BigInt(valueToSendFromRelayer)
            );
          });
          ...
          describe("When relayer don't fund the UP so it's balance is greater than the value param of execute(..)", () => {
            it("should revert", async () => {
              ...
              const targetContractPayload =
                targetContract.interface.encodeFunctionData(
      ...
      describe("when calling `executeRelayCall -> LSP0.execute(uint256,address,uint256,bytes) -> TargetContract`", () => {
        describe("when TargetContract returns an `uint256[]` array of 2 numbers", () => {
          it("should return a `bytes` that can be decoded as a `uint256[]", async () => {
            ...
            const getDynamicArrayOf2NumbersSig =
              targetContract.interface.getFunction(
                "getDynamicArrayOf2Numbers"
              ).selector;

            const erc725xExecutePayload =
              context.universalProfile.interface.encodeFunctionData("execute", [
            ...
            const expectedArrayOfNumbers =
              await targetContract.getDynamicArrayOf2Numbers();

            const [decodedUint256Array] = abiCoder.decode(
              ["uint256[]"],
              decodedResult
            );
      ...
      describe("when calling `executeRelayCall -> LSP0.executeBatch(uint256[],address[],uint256[],bytes[])` and doing 2 x STATICCALLs in the batch", () => {
        it("should return an array of `bytes[]` where each entry can be decoded individually", async () => {
          ...
          const erc725xExecuteBatchPayload =
            context.universalProfile.interface.encodeFunctionData(
              "executeBatch",
              [
          ...
          const [decodedString] = abiCoder.decode(
            ["string"],
            decodedBytesArray[0]
          );
          expect(decodedString).to.equal(expectedString);

          const [decodedNumber] = abiCoder.decode(
            ["uint256"],
            decodedBytesArray[1]
          );
        ...
        it("should return nothing 0x, set the `pendingOwner` and emit `PermissionsVerified` event with right arguments", async () => {
          ...
          await expect(tx)
            ...
            .withArgs(
              ...
              0, // value
              context.universalProfile.interface.getFunction(
                "transferOwnership"
              ).selector // selector
            );
        });
