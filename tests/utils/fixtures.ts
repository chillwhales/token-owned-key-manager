...
export async function deployProxy(
  ...
  return receipt?.contractAddress || "0x";
}
...
export async function setupKeyManager(
  ...
  const payload =
    _context.universalProfile.interface.getFunction("acceptOwnership").selector;
...
export async function grantLSP11PermissionViaKeyManager(
  ...
  const permissionArrayLength = ethers.toNumber(
    ethers.toBigInt(rawPermissionArrayLength)
  );
 ...
 */
export async function getLSP5MapAndArrayKeysValue(
  account: UniversalProfile,
  token: LSP7DigitalAsset | LSP8IdentifiableDigitalAsset
) {
  ...
  const indexInNumber = ethers.toNumber(
    ethers.toBigInt(indexInHex === "0x" ? 0 : indexInHex)
  );
  const rawIndexInArray = ethers.zeroPadValue(
    ethers.toBeHex(indexInNumber),
    16
  );
  ...
  const [arrayLength, _elementAddress] = await account.getDataBatch([
    arrayKey,
    elementInArrayKey,
  ]);
